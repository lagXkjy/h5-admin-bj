<template>
  <div>
    ...
    <div class="selectors">
      ...
      <div style="margin-left: 20px; margin-right: 10px">退款状态</div>
      ...
      <div style="margin-left: 20px; margin-right: 10px">审核结果</div>
     ...
     <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="已交押金" prop="depositMoney">
        <template slot-scope="scope">
          {{(scope.row.depositMoney/100).toFixed(2)}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="审核结果" prop="auditResult">
        <template slot-scope="scope">
          {{scope.row.auditResult == 1 ? '审核通过' : scope.row.auditResult == 2 ? '审核失败' : '-'}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primay" plain @click="jumpTo(scope.row)" v-permission="'e73b3839-9ffa-4c8b-8ce7-c11e821042fa'">查看操作</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    jumpTo(row) {
      this.$router.push({path: `/finance/auditDetail?id=${row.id}&dealStatus=${this.dealStatus}`})
    },
    setDefaultDate() {
    ...
    yieldParams() {
      ...
      params.auditResult = this.status;
