<template>
  <div>
    <el-dialog title="编辑" center :visible.sync="dialogVisible" width="70%">
      ...
      <div class="dialog_item">
        <div class="dialog_subtitle">设备图片</div>
        <el-upload class="upload-demo" drag :action="actionApi" :show-file-list="false" key="sadadsaw" :on-success="successUpload" :on-error="failedUpload" :before-upload="beforeAvatarUpload">
          <img v-if="imageUrl" :src="imageUrl" class="avatar">
          <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          <div slot="tip" style="font-size:12px">尺寸为600*600PX；大小在2M内；图片格式为PNG/JPG</div>
        </el-upload>
      </div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="typePicUrl" label="图片">
        <template slot-scope="scope">
          <img :src="scope.row.typePicUrl" alt="" style="width: 100px;" v-if="scope.row.typePicUrl">
          <span v-else>空</span>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="score2" label="大小"></el-table-column> -->
      <el-table-column prop="typePicUrl" label="输出地址">
         <template slot-scope="scope">
          <span v-if="scope.row.typePicUrl">{{scope.row.typePicUrl}}</span>
          <span v-else>空</span>
        </template>
      </el-table-column>
      <el-table-column prop="address" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" @click="editImage(scope.row)" v-permission="'1aaf67f0-c67d-44df-b3de-758867db70e0'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
